...
public class StockTest {
  ...
  /**
   * Test write access to a book that is not indexed by ISBN. Although a real bookstore should not
   * allow this, for simplicity we do not prevent setting stock for non-indexed books.
   */
  @Test
  ...
  /**
   * Test read access to a book that is not indexed by ISBN. Although a real bookstore should not
   * allow this, for simplicity we do not prevent looking up stock for non-indexed books.
   */
  @Test
  ...
  /**
   * Try a write access on a city that does not exist.
   */
  ...
  /**
   * Helper method to access the private default constructor. This test should not operate on the
   * singleton's getInstance method, since the tests may alter the bean state and therefore cannot
   * be executed in arbitrary order. To avoid this we use this method instead to ensure individual
   * tests operate on isolated instances.
   *
  ...
  private GlobalStockImpl getNewInstance() {
    try {
      ...
      throw new RuntimeException(
          "Something went wrong creating a NEW instance of CommentsImpl via reflection");
    }
  }
}
